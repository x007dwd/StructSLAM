...
namespace ygz {
    ...
    void EdgeIDPPrior::computeError() {
        ...
        //LOG(INFO) << "error = " << _error << endl;
    }

    void EdgeIDPPrior::linearizeOplus()  {
        _jacobianOplusXi.setZero();
        _jacobianOplusXi(0) = 1;
        //LOG(INFO) << "J.size: " << _jacobianOplusXi.rows() << " x " << _jacobianOplusXi.cols() << endl;
    }
    ...
    void EdgePRIDP::linearizeOplus() {
        ...
        Vector3d J_pi_rho = Rcic0 * (-d * d * P0); // (xiang) this should be squared?
    ...
    void EdgePRV::computeError() {
        ...
        if (_error.norm() > 0.1) {
            LOG(INFO) << "Error = " << _error.transpose() << endl;
        }
    }
    ...
    void EdgeProjectPoseOnly::computeError() {
        ...
        if (invz < 0) {
            // 点在后面？
            setLevel(1);
            //_error.setZero(); // if set to zero, then the edge will be recogonized as 'inlier' in poseoptimization
