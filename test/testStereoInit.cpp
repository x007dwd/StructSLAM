...
// 路径
double discardtime = 5; // discard some frames in the beginning. for V101~6s, MH01~5
...
void Tracker::TestStereoInit() {
    ...
    srand(time(nullptr));
    // create a backend
    shared_ptr<Tracker> ptracker(this);
    ...
    for (int ni = 0; ni < nImages; ni++) {
        ...
        if(ni == 115)
            ni=115;
        imLeft = cv::imread(vstrImageLeft[ni], /*CV_LOAD_IMAGE_UNCHANGED*/CV_LOAD_IMAGE_GRAYSCALE);
        imRight = cv::imread(vstrImageRight[ni], /*CV_LOAD_IMAGE_UNCHANGED*/CV_LOAD_IMAGE_GRAYSCALE);
        ...
        if (mState == eTrackingState::OK) {
            ...
            LOG(INFO) << "sleep 2000, press ctrl+c to exit" << endl;
            sleep(2000);
            break;
        }
    }
