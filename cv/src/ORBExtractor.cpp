...
#undef GET_VALUE
    ...
    void ORBExtractor::Detect(shared_ptr<Frame> frame, bool leftEye, bool computeRotAndDesc) {
        ...
        if (mMethod == FAST_MULTI_LEVEL) {
            ...
            if (leftEye) {
                ComputeKeyPointsFastSingleLevel(single_level_features, frame->mPyramidLeft[0]);
            } else {
                ComputeKeyPointsFastSingleLevel(single_level_features, frame->mPyramidRight[0]);
            }
    ...
    void ORBExtractor::ComputeKeyPointsFastSingleLevel(std::vector<shared_ptr<Feature>> &allKeypoints,
                                                       const cv::Mat &image) {
        mOccupancy = cv::Mat::zeros(image.rows, image.cols, CV_8U);

        int h = image.rows;
        int w = image.cols;
        int n = setting::extractFeatures;
        ...
        while (cnt < n) {
            if (cnt > 0) {
                mnGridSize -= 5;
                if (mnGridSize < 7) {
                    cerr << "Grid size is too small, quit: " << mnGridSize << endl;
            ...
            for (int k = 0; k < grid_n_rows * grid_n_cols; k++) {
                ...
                const uchar *data = image.ptr<uchar>(y_start) + x_start;
                vector<fast::fast_xy> fast_corners;
#ifdef __SSE2__
                fast::fast_corner_detect_10_sse2(data, mnGridSize, mnGridSize, w, setting::initTHFAST, fast_corners);
#else
                fast::fast_corner_detect_10( data, mnGridSize, mnGridSize, w, setting::initTHFAST, fast_corners );
#endif
                if (fast_corners.empty()) {
                    // try lower threshold
#ifdef __SSE2__
                    fast::fast_corner_detect_10_sse2(data, mnGridSize, mnGridSize, w, setting::minTHFAST, fast_corners);
#else
                ...
                for (fast::fast_xy &xy: fast_corners) {
                    ...
                    if (xy.x < 20 || xy.y < 20 || xy.x >= image.cols - 20 || xy.y >= image.rows - 20) {
                        // 太边缘不便于计划描述子
                        continue;
                    }
                    ...
                    if (score > setting::minShiTomasiScore)
                        corner_score.push_back(make_pair(xy, score));
                }

                if (corner_score.empty())
                    continue;
                ...
                // 每个格子最多取3个点
                pair<fast::fast_xy, float> best = *corner_score.begin();
                shared_ptr<Feature> feature = make_shared<Feature>();
                feature->mLevel = 0;
                feature->mPixel = Vector2f(best.first.x, best.first.y);
                feature->mScore = best.second;
                feature->mAngle = IC_Angle(image, feature->mPixel, umax);
    ...
    void ORBExtractor::ComputeKeyPointsGFTT(std::vector<std::vector<shared_ptr<Feature>>> &allKeypoints,
        ...
        // cv::goodFeaturesToTrack(img, pts, setting::extractFeatures, 0.01, 30, Mat(), 3, true );
        cv::goodFeaturesToTrack(img, pts, setting::extractFeatures, 0.01, 30 );
        ...
        for (cv::Point2f &pt: pts) {
            ...
            vector<size_t> indecies = mpFrame->GetFeaturesInArea( pt.x, pt.y, 20 );
            if ( indecies.empty() ) {
