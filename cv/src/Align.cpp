...
namespace ygz {
    ...
    bool Align1D(
        ...
        for (int iter = 0; iter < n_iter; ++iter) {
            int u_r = floor(u);
            int v_r = floor(v);
            if (u_r < align_halfpatch_size || v_r < align_halfpatch_size || u_r >= cur_img.cols - align_halfpatch_size ||
                v_r >= cur_img.rows - align_halfpatch_size)
                break;
            ...
            Jres.setZero();
            for (int y = 0; y < align_patch_size; ++y) {
                uint8_t *it = (uint8_t *) cur_img.data + (v_r + y - align_halfpatch_size) * cur_step + u_r - align_halfpatch_size;
