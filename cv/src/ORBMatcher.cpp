...
namespace ygz {
    ...
    void ORBMatcher::ComputeStereoMatchesORB(shared_ptr<Frame> f) {
        ...
        for (size_t iL = 0; iL < N; iL++) {
            ...
            if (bestDist < thOrbDist) {
                ...
                if (disparity >= minD && disparity < maxD) {
                    ...
                    f->mFeaturesLeft[iL]->mfRightU = bestuR;
        ...
        for (int i = vDistIdx.size() - 1; i >= 0; i--) {
            if (vDistIdx[i].first < thDist) {
                ...
                f->mFeaturesLeft[vDistIdx[i].second]->mfRightU = -1;
            }
        }
    }

    void ORBMatcher::ComputeStereoMatchesOptiFlow(shared_ptr<Frame> f) {
    ...
    void ORBMatcher::ComputeStereoMatchesOptiFlowCV(shared_ptr<Frame> f) {
        ...
        for (size_t i = 0; i < rightPts.size(); i++) {
            if (status[i]) {
                // lk succeed
                shared_ptr<Feature> &feat = f->mFeaturesLeft[i];
                if (feat->mpPoint)    // already have a point
                    continue;
    ...
    void ORBMatcher::ComputeStereoMatchesSpasePoint(shared_ptr<Frame> f) {
    ...
    int ORBMatcher::SearchByDirectProjection(shared_ptr<Frame> F, const std::set<shared_ptr<MapPoint>> &vpMapPoints) {
        ...
        for (const shared_ptr<MapPoint> &mp: vpMapPoints) {
            ...
            assert(idxFeatKF >= 0 && idxFeatKF < kf->mFeaturesLeft.size());
            shared_ptr<Feature> refFeat = kf->mFeaturesLeft[idxFeatKF];

            Eigen::Matrix2d ACR;
            Vector2f px_ref = refFeat->mPixel;
            SE3d pose_ref = SE3d(kf->mRcw, kf->mtcw);
            SE3d TCR = SE3d(F->mRcw, F->mtcw) * pose_ref.inverse();

            // 计算带边界的affine wrap，边界是为了便于计算梯度
            this->GetWarpAffineMatrix(kf, F, refFeat, refFeat->mLevel, TCR, ACR);

            // int search_level = GetBestSearchLevel(ACR, kf->mPyramidLeft.size() - 1, ref);
            int search_level = 0;
    ...
    void ORBMatcher::ComputeDistinctiveDescriptors(shared_ptr<MapPoint> mp) {
        ...
        // 获取所有描述
        auto observations = mp->mObservations;
